...
class XBlockChildren(DeveloperErrorViewMixin, APIView):
    """
    Batch-render selected child XBlocks under a parent (library_content / item_bank / vertical).

    **Use Cases**

        After a shell-mode ``render_xblock`` response, the Learning MFE (or a curl client)
        loads CAPA children in batches via this endpoint.

    **Example Request**

        GET /api/courseware/v1/xblock_children/?parent_usage_key=...&child_usage_keys=key1,key2

    **Response Values**

        * parent_usage_key: echo of the parent key
        * results: list of {usage_key, html, resources} for successfully rendered children
        * errors: list of {usage_key, error, message} for forbidden / missing / failed children

    **Returns**

        * 200 on success (individual child failures appear in ``errors``)
        * 400 for missing/invalid params or oversized batch
        * 401/403 when unauthenticated or lacking course access
        * 404 when the parent cannot be loaded
    """
    ...
    def get(self, request, *args, **kwargs):  # lint-amnesty, pylint: disable=unused-argument
        ...
        from lms.djangoapps.courseware.block_render import render_xblock_children
        from lms.djangoapps.courseware.exceptions import CourseAccessRedirect

        parent_key_str = request.query_params.get('parent_usage_key')
        child_keys_raw = request.query_params.get('child_usage_keys', '')
        if not parent_key_str:
            return Response(
                {'developer_message': 'parent_usage_key is required'},
                status=status.HTTP_400_BAD_REQUEST,
            )

        try:
            parent_usage_key = UsageKey.from_string(parent_key_str)
        except InvalidKeyError:
            return Response(
                {'developer_message': 'Invalid parent_usage_key'},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        for key_str in child_key_strings:
            try:
                child_usage_keys.append(UsageKey.from_string(key_str))
            except InvalidKeyError:
                return Response(
                    {'developer_message': f'Invalid child_usage_key: {key_str}'},
                    status=status.HTTP_400_BAD_REQUEST,
                )

        try:
            course = get_course_with_access(
                request.user, 'load', parent_usage_key.course_key, check_if_enrolled=True
            )
        except CourseAccessRedirect:
            raise NotFound()  # lint-amnesty, pylint: disable=raise-missing-from

        try:
            payload = render_xblock_children(
                request,
                parent_usage_key,
                child_usage_keys,
                course=course,
        ...
        # If every requested child was forbidden, surface 403 for clearer client handling.
        if (
            child_usage_keys
            and not payload['results']
            and payload['errors']
            and all(err.get('error') == 'forbidden' for err in payload['errors'])
        ):
