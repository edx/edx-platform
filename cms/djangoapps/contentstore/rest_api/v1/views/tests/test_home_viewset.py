"""
Unit tests for HomeViewSet (ADR 0028).
...
patch.object is used for the ViewSet's get_serializer() method because:
  ...
  - Patching get_serializer() lets us focus on routing + service-call assertions
    without re-testing serializer logic (covered in serializer unit tests).
"""
...
from cms.djangoapps.contentstore.rest_api.v1.views.home import HomeViewSet
...
MOCK_GET_HOME_CONTEXT = (
    'cms.djangoapps.contentstore.rest_api.v1.views.home.get_home_context'
)
MOCK_GET_COURSE_CONTEXT = (
    'cms.djangoapps.contentstore.rest_api.v1.views.home.get_course_context'
)
MOCK_GET_LIBRARY_CONTEXT = (
    'cms.djangoapps.contentstore.rest_api.v1.views.home.get_library_context'
)
MOCK_ORG_API = (
    'cms.djangoapps.contentstore.rest_api.v1.views.home.org_api'
)


class TestHomeViewSetPermissions(APITestCase):
    """
    ADR 0028 – permission regression tests for HomeViewSet.

    Verifies that IsAuthenticated enforces the same access rules previously
    provided by @view_auth_classes(is_authenticated=True) on the deprecated views.
    """

    def test_unauthenticated_list_returns_401(self):
        """Unauthenticated GET /home/ must return 401."""
        url = reverse('cms.djangoapps.contentstore:v1:home-list')
        response = self.client.get(url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_unauthenticated_courses_returns_401(self):
        """Unauthenticated GET /home/courses/ must return 401."""
        url = reverse('cms.djangoapps.contentstore:v1:home-courses')
        response = self.client.get(url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_unauthenticated_libraries_returns_401(self):
        """Unauthenticated GET /home/libraries/ must return 401."""
        url = reverse('cms.djangoapps.contentstore:v1:home-libraries')
        response = self.client.get(url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestHomeViewSetActions(APITestCase):
    ...
    def test_list_calls_get_home_context(self, mock_home, mock_org, mock_get_ser):
        ...
        response = self.client.get(reverse('cms.djangoapps.contentstore:v1:home-list'))

        assert response.status_code == status.HTTP_200_OK
        mock_home.assert_called_once()

    # ------------------------------------------------------------------
    # courses → GET /home/courses/
    # ------------------------------------------------------------------
    ...
    def test_courses_calls_get_course_context(self, mock_courses, mock_get_ser):
        ...
        mock_get_ser.return_value.data = {'courses': [], 'archived_courses': [], 'in_process_course_actions': []}

        response = self.client.get(reverse('cms.djangoapps.contentstore:v1:home-courses'))

        assert response.status_code == status.HTTP_200_OK
        mock_courses.assert_called_once()

    # ------------------------------------------------------------------
    # libraries → GET /home/libraries/
    # ------------------------------------------------------------------
    ...
    def test_libraries_calls_get_library_context(self, mock_libs, mock_get_ser):
        ...
        response = self.client.get(reverse('cms.djangoapps.contentstore:v1:home-libraries'))
