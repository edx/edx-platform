...
class CertificatesViewsTests(CommonCertificatesTestCase, CacheIsolationTestCase):
    ...
    def test_certificate_pdf_download_shows_proctoring_block(self, _mock_proctoring_status):
        ...
        self.assertContains(response, 'Complete your required proctored exam', status_code=403)
    ...
    def test_certificate_pdf_download_streams_when_host_is_allowed_by_storage_settings(
        ...
        upstream_response.iter_content.return_value = iter([b'%PDF'])
    ...
    @override_settings(
        ...
        AWS_STORAGE_BUCKET_NAME='certificate-bucket',
        AWS_S3_REGION_NAME='eu-west-1',
    )
    @patch('lms.djangoapps.certificates.views.webview.requests.get')
    def test_certificate_pdf_download_allows_regional_storage_host(
        self, mock_get, _mock_proctoring_status
    ):
        self.cert.download_url = (
            'https://certificate-bucket.s3.eu-west-1.amazonaws.com/certificates/download'
        )
        self.cert.save()
        upstream_response = Mock()
        upstream_response.status_code = 200
    ...
    @patch('lms.djangoapps.certificates.views.webview.requests.get')
    def test_certificate_pdf_download_accepts_signature_split_across_chunks(
        self, mock_get, _mock_proctoring_status
    ):
        upstream_response = Mock()
        upstream_response.status_code = 200
        upstream_response.headers = {'Content-Type': 'application/pdf'}
        upstream_response.iter_content.return_value = iter([b'%P', b'DF-1.7'])
        mock_get.return_value = upstream_response
    ...
    def test_certificate_pdf_download_returns_404_for_different_user(self):
        ...
        other_user.set_password('foo')
        other_user.save()
        self.client.login(username=other_user.username, password='foo')
    ...
    def test_certificate_pdf_download_returns_503_for_non_pdf_response(self, mock_get, _mock_proctoring_status):
        ...
        upstream_response.status_code = 200
        upstream_response.headers = {'Content-Type': 'application/pdf'}
        upstream_response.iter_content.return_value = iter([b'<html>'])
        mock_get.return_value = upstream_response
