...
def _allowed_certificate_pdf_hosts():
    ...
    endpoint_host = urllib.parse.urlparse(endpoint_url).hostname
    if endpoint_host:
    ...
    if bucket_name and not bucket_name.startswith('SET-ME-PLEASE'):
        ...
        if region:
            hosts.add(f'{bucket_name}.s3.{region}.amazonaws.com')
            hosts.add(f'{bucket_name}.s3-{region}.amazonaws.com')

    normalized_hosts = set()
    for host in hosts:
        parsed_host = urllib.parse.urlparse(host if '://' in host else f'//{host}').hostname
        if parsed_host:
            normalized_hosts.add(parsed_host.lower())
...
def _read_pdf_prefix(content_iter):
    """Read enough streamed content to validate the PDF signature."""
    buffered_chunks = []
    pdf_prefix = b''
    for chunk in content_iter:
        ...
        pdf_prefix += chunk
        if len(pdf_prefix) >= len(b'%PDF'):
            break
    return buffered_chunks, pdf_prefix
...
def _stream_certificate_pdf(certificate):
    ...
    try:
        buffered_chunks, pdf_prefix = _read_pdf_prefix(content_iter)
    except requests.exceptions.RequestException:
        upstream_response.close()
        log.exception(
    ...
    if not pdf_prefix.startswith(b'%PDF'):
        upstream_response.close()
        log.error(
