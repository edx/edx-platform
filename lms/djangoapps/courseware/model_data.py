...
def _children_for_field_data_cache(block, read_only=False):
    """
    ...
    get_child_blocks_for_prefetch = getattr(block, 'get_child_blocks_for_prefetch', None)
    has_dynamic_children = getattr(block, 'has_dynamic_children', None)
    if callable(has_dynamic_children) and has_dynamic_children():
        if read_only and callable(get_child_blocks_for_prefetch):
            return list(get_child_blocks_for_prefetch())

        if callable(get_child_blocks):
    ...
    def add_block_descendents(self, block, depth=None, block_filter=lambda block: True):
        ...
        def collect_descendant_blocks(block, depth, block_filter):
            ...
            if depth is None or depth > 0:
                ...
                for child in _children_for_field_data_cache(block, read_only=self.read_only):
                    if child is None:
                        continue
