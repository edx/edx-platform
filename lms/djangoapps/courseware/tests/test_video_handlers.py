...
class TestVideoBlockAudioDescriptionPlayback(TestVideo):  # lint-amnesty, pylint: disable=test-inherits-tests
    """
    ...
    @patch('xmodule.video_block.video_block.edxval_api.get_video_audio_description_url', create=True)
    def test_playback_url_returns_presigned_url(self, mock_generate):
        ...
        self.block.edx_video_id = 'video-1'

        url = self.block._get_audio_description_url()  # pylint: disable=protected-access

        assert url == 'https://s3.example/get-presigned'
        mock_generate.assert_called_once_with('video-1')

    def test_playback_url_returns_none_when_no_ad(self):
        """
        Without an AD filename or edx_video_id, the helper short-circuits
        to None (no S3 round-trip).
        """
        self.block.audio_description = ''
        url = self.block._get_audio_description_url()  # pylint: disable=protected-access
        assert url is None
