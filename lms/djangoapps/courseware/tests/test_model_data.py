...
class TestFieldDataCacheDynamicChildren(TestCase):
    ...
    def test_children_for_field_data_cache_uses_prefetch_api_in_read_only_mode(self):
        """
        Read-only prefetch should use a side-effect-free dynamic-child API when available.
        """
        selected_child = Mock(name='selected_child')
        dynamic_block = Mock(name='dynamic_block')
        dynamic_block.has_dynamic_children.return_value = True
        dynamic_block.get_child_blocks_for_prefetch.return_value = [selected_child]
        dynamic_block.get_child_blocks.side_effect = AssertionError(
            'get_child_blocks should not be called for read-only prefetch'
        )
        dynamic_block.get_children.side_effect = AssertionError(
            'get_children should not be called for dynamic blocks'
        )

        assert _children_for_field_data_cache(dynamic_block, read_only=True) == [selected_child]
        dynamic_block.get_child_blocks_for_prefetch.assert_called_once_with()
    ...
    def test_add_block_descendents_prefetches_only_selected_dynamic_children(self, mock_modulestore):
        ...
        library_content.location = Mock(course_key=COURSE_KEY)
        ...
        vertical.location = Mock(course_key=COURSE_KEY)
