...
from ..toggles import (
    course_home_mfe_track_selection_is_active,
    learner_can_preview_verified_content,
)


class TestCourseHomeMfeTrackSelectionToggle(TestCase):
    """Test cases for course_home_mfe_track_selection_is_active."""

    def setUp(self):
        self.course_key = CourseKey.from_string("course-v1:TestX+CS101+2024")

    @patch("lms.djangoapps.course_home_api.toggles.COURSE_HOME_MICROFRONTEND_TRACK_SELECTION.is_enabled")
    def test_returns_true_when_waffle_enabled(self, mock_is_enabled):
        mock_is_enabled.return_value = True
        self.assertTrue(course_home_mfe_track_selection_is_active(self.course_key))

    @patch("lms.djangoapps.course_home_api.toggles.COURSE_HOME_MICROFRONTEND_TRACK_SELECTION.is_enabled")
    def test_returns_false_when_waffle_disabled(self, mock_is_enabled):
        mock_is_enabled.return_value = False
        self.assertFalse(course_home_mfe_track_selection_is_active(self.course_key))
    ...
    def test_returns_false_for_deprecated_course_key(self, mock_is_enabled):
        ...
        self.assertFalse(course_home_mfe_track_selection_is_active(deprecated_key))
